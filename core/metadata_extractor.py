...
class MetadataExtractor:
    ...
    def extract_metadata(self, soup: BeautifulSoup, url: str) -> Dict:
        ...
        metadata['publication_name'] = self._extract_publication_name(soup, metadata.get('source'))
    ...
    def _extract_top_image(self, soup: BeautifulSoup, url: str) -> str:
        ...
        content_images = soup.select('article img, .content img, .post-content img, .entry-content img')
            ...
            for script in jsonld_scripts:
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        if data.get('@type') in ['Article', 'NewsArticle'] and 'image' in data:
                            image = data['image']
    ...
    def _is_valid_image(self, img_element) -> bool:
        ...
        skip_patterns = [
            ...
            'advertisement', 'ad-', 'banner', 'placeholder', 'spacer',
    ...
    def _extract_opengraph_metadata(self, soup: BeautifulSoup) -> Dict:
        ...
        og_properties = [
            ...
            'og:type', 'og:site_name', 'article:author',
                        ...
                        if data.get('@type') in ['Article', 'NewsArticle']:
                            ...
                            if 'author' in data:
                                if isinstance(data['author'], dict):
                                    jsonld_metadata['jsonld_author'] = data['author'].get('name', '')
                                elif isinstance(data['author'], str):
                                    jsonld_metadata['jsonld_author'] = data['author']
                            if 'datePublished' in data:
                                jsonld_metadata['jsonld_date_published'] = data['datePublished']
                            if 'description' in data:
                                jsonld_metadata['jsonld_description'] = data['description']
    ...
    def _extract_category(self, soup: BeautifulSoup) -> str:
        ...
        breadcrumb = soup.select_one('.breadcrumb a, .breadcrumbs a, .b-breadcrumbs__item a')
        if breadcrumb:
    ...
    def _extract_tags(self, soup: BeautifulSoup, metadata: Dict) -> List[str]:
        ...
        tag_selectors = [
            ...
            '.tag-links a', '.entry-tags a', '.category-tags a',
            '.tags span', '.post-tags span', '.article-tags span',
            '.wp-tag-cloud a', '.tag-list a', '.hashtags a'
        ]
        ...
        if not tags:
            breadcrumbs = soup.select('.breadcrumb a, .breadcrumbs a, .nav-breadcrumb a')
            for breadcrumb in breadcrumbs[1:]:  # Skip first (usually "Home")
                breadcrumb_text = breadcrumb.get_text(strip=True)
                if breadcrumb_text and breadcrumb_text.lower() not in ['home', 'news', 'articles']:
    ...
    def _extract_image_urls(self, soup: BeautifulSoup) -> list[str]:
        ...
        images = []
        article_body = soup.find("article") or soup.find("div", class_="post-content") or soup.find("div", class_="entry-content")
        if article_body:
