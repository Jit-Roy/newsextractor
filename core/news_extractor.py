...
class NewsExtractor:
    ...
    def __init__(self, 
        ...
        self.nlp_processor = NLPProcessor(enable_transformers=enable_transformers, summarization_method=summarization_method) if enable_nlp else None
    ...
    def extract_from_rss_feed(self, feed_url: str, limit: Optional[int] = None) -> List[Article]:
        """
        ...
        try:
            ...
            self.logger.info(f"Successfully extracted {len(articles)} articles from RSS feed")
            return articles
    ...
    def _create_article_from_data(self, data: Dict) -> Optional[Article]:
        ...
        try:
            ...
            if not data.get('title') or not data.get('content'):
                self.logger.warning(f"Skipping article due to missing title or content: {data.get('url')}")
                return None
            ...
            article = Article(
                ...
                language=processed_data.get('language', 'unknown')  # Set detected language
            )
    ...
    def _process_language_and_nlp(self, article: Article):
        ...
        try:
            ...
            if self.language and not article.translated:
                ...
                try:
                    if article.content:
                        translated_content = translator.translate(article.content, target_lang=self.language)
                        article.content = translated_content
                    if article.title:
                        translated_title = translator.translate(article.title, target_lang=self.language)
                        article.title = translated_title
                    article.translated = True
                    self.logger.info(f"Article translated to {self.language}")
                except Exception as e:
                    self.logger.warning(f"Translation failed: {e}. Continuing without translation.")
            
            # NLP processing
            if self.enable_nlp and self.nlp_processor:
                nlp_results = self.nlp_processor.process_article(article.title, article.content)
                ...
                self.logger.debug(f"NLP processing completed for article: {sum(len(v) for v in nlp_results.entities.values())} entities")
