...
class Translator:
    ...
    def translate(self, 
        ...
        if source_lang == target_lang:
            self.logger.debug(f"Skipping translation: source ({source_lang}) == target ({target_lang})")
            return text
    ...
    def _translate_with_provider(self, 
                               text: str, 
                               target_lang: str, 
    ...
    def _translate_google_free(self, 
                              text: str, 
                              target_lang: str, 
                              source_lang: str) -> TranslationResult:
        """
            ...
            for base_url in endpoints:
                try:
                    ...
                    if result and len(result) > 0 and result[0]:
                        translated_text = ''.join([sentence[0] for sentence in result[0] if sentence and sentence[0]])
    ...
    def _translate_google_paid(self, 
                              text: str, 
                              target_lang: str, 
                              source_lang: str) -> TranslationResult:
        """
        ...
        """
        if not self.api_key:
            raise TranslationError("Google Cloud API key is required for paid translation")
        ...
        try:
            ...
            if result and len(result) > 0:
                ...
                return TranslationResult(
                    ...
                    source_language=translation.get('detectedLanguage', {}).get('language', source_lang),
                    target_language=target_lang,
                    confidence=translation.get('detectedLanguage', {}).get('score', 0.9),
                    provider="microsoft"
                )
            else:
        ...
        try:
            ...
            if 'translations' in result and len(result['translations']) > 0:
                ...
                return TranslationResult(
                    ...
                    source_language=translation.get('detected_source_language', source_lang),
                    target_language=target_lang,
    ...
    def _is_latin_script(self, text: str) -> bool:
        ...
        latin_chars = sum(1 for char in cleaned_text if ord(char) < 592)  # Covers most Latin scripts
